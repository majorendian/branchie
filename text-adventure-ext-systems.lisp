...
(defmethod enemy-battle-simple ((player_sym symbol) (enemy_sym symbol) (branch_name symbol) (victory_text string) (victory_branch branch) &key bg-image dmg-image)
                 ...
                 (let ((roll (random (gc-att (player))))) 
                   (if (< (gc-def (enemy)) roll)
                     (progn
                       ...
                       (if (<= (gc-hp (enemy)) 0)
                         (progn
                           (set-branch-text b victory_text)
                           (set-branch-next b victory_branch))
                         (progn
